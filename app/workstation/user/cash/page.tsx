  ...
  return <div className="space-y-6">
    ...
    {registers.length === 0 ? <EmptyCash /> : <>
      ...
      {!shift ? <OpenShift register={selectedRegister} openingAmount={openingAmount} setOpeningAmount={setOpeningAmount} onSubmit={() => void startShift()} saving={saving} /> : <>
        ...
        <WaitingSalesPanel sales={waitingSales} onResume={resumeWaiting} saving={saving} />
...
function WaitingSalesPanel({ sales, onResume, saving }: { sales: Sale[]; onResume: (id: string) => Promise<void>; saving: boolean }) { return <Card><CardHeader className="flex-row items-start justify-between gap-4"><div><CardTitle>Cuentas en espera ({sales.length})</CardTitle><CardDescription>Ventas pausadas de este turno. No afectan el efectivo hasta cobrarlas.</CardDescription></div><Badge variant={sales.length ? 'warning' : 'neutral'}>{sales.length ? 'Pendientes' : 'Sin pendientes'}</Badge></CardHeader><CardContent>{sales.length === 0 ? <p className="rounded-lg border border-dashed p-6 text-center text-sm text-muted-foreground">No hay cuentas en espera en este turno.</p> : <div className="space-y-2">{sales.map((sale) => <div key={sale.id} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border p-3"><div><p className="font-medium">#{sale.id} · {sale.client?.name || 'Cliente'}</p><p className="text-xs text-muted-foreground">{sale.pet?.name || 'Sin mascota'} · {dateTime(sale.createdAt)}</p></div><div className="flex items-center gap-3"><span className="font-semibold">{money(sale.total)}</span><Button size="sm" onClick={() => void onResume(String(sale.id))} disabled={saving}>Retomar</Button></div></div>)}</div>}</CardContent></Card> }
