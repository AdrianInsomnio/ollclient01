...
export async function assignConsultorio(
  id: string | number,
  data: ConsultorioAssignmentPayload,
): Promise<Consultation> {
  const response = await patch<ConsultationResponse>(
    `/consultations/${id}/consultorio`,
    data,
  );
  return response.consultation;
}

export async function releaseConsultorio(
  id: string | number,
): Promise<Consultation> {
  const response = await del<ConsultationResponse>(
    `/consultations/${id}/consultorio`,
  );
  return response.consultation;
}
...
export async function updateConsultationPriority(
  id: string | number,
  priority: Consultation["priority"],
): Promise<Consultation> {
  const response = await put<ConsultationResponse>(`/consultations/${id}`, {
    priority,
  });
  return response.consultation;
}
...
export async function openConsultation(
  ...
  const response = await post<OpenConsultationResponse | Consultation>(
    "/consultations",
    {
      ...data,
  ...
  return "consultation" in response && response.consultation
    ...
    : (response as Consultation);
}

export async function closeConsultation(
  id: string,
  data: CloseConsultationPayload,
): Promise<CloseConsultationResponse> {
  const idempotencyKey = generateIdempotencyKey();
  return post<CloseConsultationResponse>(
...
export async function addConsultationItem(
  consultationId: string,
  data: AddConsultationItemPayload,
): Promise<Consultation> {
  const response = await post<ConsultationResponse>(
    "/consultations/" + consultationId + "/items",
    data,
  );
  return response.consultation;
}
...
export async function updateConsultationClinical(
  id: string,
  data: UpdateClinicalPayload,
): Promise<Consultation> {
  const response = await put<ConsultationResponse>(
    "/consultations/" + id,
    data,
  );
  return response.consultation;
...
export async function addDiagnosis(
  id: string,
  description: string,
): Promise<{ id: string | number; description: string }> {
