  ...
  return (
    <div className="space-y-8">
      ...
      <Tabs value={tab} onValueChange={(value) => setTab(value as Tab)}>
        ...
        <TabsContent value="services" className="mt-5"><ServicesTable services={servicesQuery.data ?? []} onEdit={openEditService} onRefresh={() => void invalidate()} /></TabsContent>
      ...
      <ServiceDialog key={`service-${editingService?.id ?? 'new'}`} open={dialog === 'service'} service={editingService} onOpenChange={(open) => !open && setDialog(null)} onSaved={() => { setDialog(null); void invalidate() }} />
...
function ServicesTable({ services, onEdit, onRefresh }: { services: Service[]; onEdit: (service: Service) => void; onRefresh: () => void }) {
  const [search, setSearch] = useState(''); const [status, setStatus] = useState('all')
  const filtered = services.filter((service) => service.name.toLowerCase().includes(search.toLowerCase()) && (status === 'all' || (status === 'active' ? service.isActive : !service.isActive)))
  ...
  return <Card><CardHeader><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center"><CardTitle className="text-base">Catálogo de servicios</CardTitle><div className="flex gap-2"><div className="relative"><Search className="absolute left-2.5 top-2 size-4 text-muted-foreground" /><Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar servicio" className="pl-8 sm:w-64" /></div><NativeFilter value={status} onChange={setStatus} options={[['all', 'Todos'], ['active', 'Activos'], ['inactive', 'Inactivos']]} /></div></div></CardHeader><CardContent className="p-0"><Table><TableHeader><TableRow>{['Servicio', 'Categoría', 'Duración', 'Precio', 'Estado', 'Acciones'].map((head) => <TableHead key={head}>{head}</TableHead>)}</TableRow></TableHeader><TableBody>{filtered.length === 0 ? <EmptyRow colSpan={6} text={services.length ? 'No hay servicios para estos filtros.' : 'No hay servicios registrados'} /> : filtered.map((service) => <TableRow key={service.id}><TableCell><div className="font-medium">{service.name}</div><div className="max-w-56 truncate text-xs text-muted-foreground">{service.description || 'Sin descripción'}</div></TableCell><TableCell>{service.category || '—'}</TableCell><TableCell>{service.duration ? `${service.duration} min` : '—'}</TableCell><TableCell>{money(service.price)}</TableCell><TableCell><Badge variant={service.isActive ? 'success' : 'neutral'}>{service.isActive ? 'Activo' : 'Inactivo'}</Badge></TableCell><TableCell><DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon-sm" aria-label={`Acciones de ${service.name}`}><Ellipsis /></Button></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuItem onClick={() => onEdit(service)}>Editar</DropdownMenuItem><DropdownMenuItem onClick={() => statusMutation.mutate({ id: service.id, isActive: !service.isActive })}>{service.isActive ? 'Desactivar' : 'Activar'}</DropdownMenuItem></DropdownMenuContent></DropdownMenu></TableCell></TableRow>)}</TableBody></Table></CardContent></Card>
}
...
function ProductDialog({ open, product, categories, onOpenChange, onSaved }: { open: boolean; product: Product | null; categories: ProductCategory[]; onOpenChange: (open: boolean) => void; onSaved: () => void }) {
  const [form, setForm] = useState<ProductPayload>(() => productForm(product)); const [error, setError] = useState('')
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl"><DialogHeader><DialogTitle>{product ? 'Editar producto' : 'Nuevo producto'}</DialogTitle><DialogDescription>Los datos se guardan en la clínica activa del usuario autenticado.</DialogDescription></DialogHeader><form onSubmit={submit} className="space-y-5"><div className="grid gap-4 sm:grid-cols-2"><Field label="Nombre *"><Input value={form.name} onChange={(event) => update('name', event.target.value)} /></Field><Field label="SKU"><Input value={form.sku ?? ''} onChange={(event) => update('sku', event.target.value || undefined)} /></Field><Field label="Categoría"><Select value={form.categoryId ? String(form.categoryId) : 'none'} onValueChange={(value) => update('categoryId', value === 'none' ? undefined : Number(value))}><SelectTrigger><SelectValue placeholder="Sin categoría" /></SelectTrigger><SelectContent><SelectItem value="none">Sin categoría</SelectItem>{categories.filter((category) => category.isActive).map((category) => <SelectItem key={category.id} value={String(category.id)}>{category.name}</SelectItem>)}</SelectContent></Select></Field><Field label="Descripción"><Textarea value={form.description ?? ''} onChange={(event) => update('description', event.target.value || undefined)} rows={2} /></Field></div><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><Field label="Precio *"><Input type="number" min="0" step="0.01" value={form.price} onChange={(event) => update('price', Number(event.target.value))} /></Field><Field label="Costo"><Input type="number" min="0" step="0.01" value={form.cost ?? ''} onChange={(event) => update('cost', event.target.value === '' ? undefined : Number(event.target.value))} /></Field><Field label="Stock inicial"><Input type="number" min="0" step="1" value={form.stock} onChange={(event) => update('stock', Number(event.target.value))} /></Field><Field label="Stock mínimo"><Input type="number" min="0" step="1" value={form.minStock} onChange={(event) => update('minStock', Number(event.target.value))} /></Field></div><CheckboxField label="Producto activo" checked={form.isActive} onChange={(checked) => update('isActive', checked)} />{error && <p className="text-sm text-destructive">{error}</p>}<DialogFooter><Button type="button" variant="outline" onClick={() => onOpenChange(false)}>Cancelar</Button><Button type="submit" disabled={mutation.isPending}>{mutation.isPending && <RefreshCw className="animate-spin" />} {product ? 'Guardar cambios' : 'Crear producto'}</Button></DialogFooter></form></DialogContent></Dialog>
}

function ServiceDialog({ open, service, onOpenChange, onSaved }: { open: boolean; service: Service | null; onOpenChange: (open: boolean) => void; onSaved: () => void }) {
  const [form, setForm] = useState<ServicePayload>(() => serviceForm(service)); const [error, setError] = useState('')
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="sm:max-w-lg"><DialogHeader><DialogTitle>{service ? 'Editar servicio' : 'Nuevo servicio'}</DialogTitle><DialogDescription>Solo se utilizan campos existentes en Service.</DialogDescription></DialogHeader><form onSubmit={submit} className="space-y-4"><Field label="Nombre *"><Input value={form.name} onChange={(event) => update('name', event.target.value)} /></Field><div className="grid gap-4 sm:grid-cols-2"><Field label="Categoría"><Input value={form.category ?? ''} onChange={(event) => update('category', event.target.value || undefined)} /></Field><Field label="Precio *"><Input type="number" min="0" step="0.01" value={form.price} onChange={(event) => update('price', Number(event.target.value))} /></Field><Field label="Duración (minutos)"><Input type="number" min="1" step="1" value={form.duration ?? ''} onChange={(event) => update('duration', event.target.value === '' ? undefined : Number(event.target.value))} /></Field></div><Field label="Descripción"><Textarea value={form.description ?? ''} onChange={(event) => update('description', event.target.value || undefined)} rows={3} /></Field><CheckboxField label="Servicio activo" checked={form.isActive} onChange={(checked) => update('isActive', checked)} />{error && <p className="text-sm text-destructive">{error}</p>}<DialogFooter><Button type="button" variant="outline" onClick={() => onOpenChange(false)}>Cancelar</Button><Button type="submit" disabled={mutation.isPending}>{mutation.isPending && <RefreshCw className="animate-spin" />} {service ? 'Guardar cambios' : 'Crear servicio'}</Button></DialogFooter></form></DialogContent></Dialog>
}
...
function CategoryDialog({ open, category, onOpenChange, onSaved }: { open: boolean; category: ProductCategory | null; onOpenChange: (open: boolean) => void; onSaved: () => void }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="sm:max-w-lg"><DialogHeader><DialogTitle>{category ? 'Editar categoría' : 'Nueva categoría'}</DialogTitle></DialogHeader><form onSubmit={submit} className="space-y-4"><Field label="Nombre *"><Input value={name} onChange={(event) => setName(event.target.value)} /></Field><Field label="Descripción"><Textarea value={description} onChange={(event) => setDescription(event.target.value)} rows={3} /></Field>{error && <p className="text-sm text-destructive">{error}</p>}<DialogFooter><Button type="button" variant="outline" onClick={() => onOpenChange(false)}>Cancelar</Button><Button type="submit" disabled={mutation.isPending}>{mutation.isPending && <RefreshCw className="animate-spin" />} Guardar</Button></DialogFooter></form></DialogContent></Dialog>
}
