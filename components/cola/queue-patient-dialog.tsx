...
export function QueuePatientDialog({
  ...
  const initialStart = consultation?.startAt
    ? new Date(consultation.startAt)
    : new Date();
  const [startAt, setStartAt] = useState(() => toLocalInput(initialStart));
  const [endAt, setEndAt] = useState(() =>
    toLocalInput(
      consultation?.endAt
        ? new Date(consultation.endAt)
        : new Date(initialStart.getTime() + 60 * 60_000),
    ),
  );
  const [consultorioId, setConsultorioId] = useState(() =>
    consultation?.consultorioId ? String(consultation.consultorioId) : "",
  );
  const [deleteOpen, setDeleteOpen] = useState(false);
  const [rescheduleOpen, setRescheduleOpen] = useState(false);
  const [rescheduleDate, setRescheduleDate] = useState(() =>
    toLocalInput(new Date(Date.now() + 24 * 60 * 60_000)).slice(0, 10),
  );
  const [rescheduleTime, setRescheduleTime] = useState(() =>
    toLocalInput(new Date()).slice(11, 16),
  );
  ...
  const assignMutation = useMutation({
    mutationFn: () => {
      if (!consultation || !consultorioId)
        throw new Error("Selecciona un consultorio.");
  ...
  const rescheduleMutation = useMutation({
    mutationFn: async () => {
      ...
      if (!rescheduleDate || !rescheduleTime)
        throw new Error("Selecciona la nueva fecha y hora.");
  ...
  const activeConsultorios = (consultoriosQuery.data ?? []).filter(
  ...
  return (
    <>
          ...
          {consultation && (
            <>
              <DialogHeader>
                ...
                <DialogDescription>
                  Información del paciente y gestión de la atención.
                </DialogDescription>
              </DialogHeader>
              ...
              <div className="grid gap-3 rounded-lg border bg-muted/20 p-4 sm:grid-cols-2">
                ...
                >
                  {consultation.client?.name ||
                    `Cliente #${consultation.clientId}`}
                </InfoItem>
                <InfoItem
                  icon={<PawPrint className="size-4" />}
                  label="Paciente"
                >
                  {consultation.pet?.species || "Mascota"}
                  {consultation.pet?.breed
                    ? ` · ${consultation.pet.breed}`
                    : ""}
                </InfoItem>
                <InfoItem icon={<Clock3 className="size-4" />} label="En cola">
                ...
                <InfoItem
                  ...
                  label="Consultorio"
                >
                  {consultation.consultorio?.name || "Sin asignar"}
                </InfoItem>
                <InfoItem label="Motivo" className="sm:col-span-2">
                  {consultation.notes || "Consulta general"}
                </InfoItem>
              </div>
              ...
              <div className="space-y-4 rounded-lg border p-4">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      Define el espacio y el horario de atención.
                    </p>
                  </div>
                </div>
                <div className="grid gap-3 sm:grid-cols-3">
                  <div className="space-y-1.5 sm:col-span-3">
                    ...
                    <select
                      id="queue-consultorio"
                      value={consultorioId}
                      onChange={(event) => setConsultorioId(event.target.value)}
                      className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
                      disabled={
                        consultoriosQuery.isLoading || assignMutation.isPending
                      }
                    >
                      <option value="">Seleccionar consultorio...</option>
                      {activeConsultorios.map((item) => (
              ...
              <DialogFooter className="flex-wrap sm:justify-between">
                <div className="flex flex-wrap gap-2">
                  <Button asChild type="button" variant="outline">
                    <Link
                      href={`/workstation/user/mascotas/${consultation.petId}`}
                    >
                ...
                {consultation.consultorioId && (
                  <Button
                    ...
                    type="button"
                    disabled={
                      assignMutation.isPending || deleteMutation.isPending
                    }
                  >
                    <Link
                      href={`/workstation/user/consultas/${consultation.id}`}
      ...
      <Dialog
        open={rescheduleOpen}
        onOpenChange={(value) =>
          !rescheduleMutation.isPending && setRescheduleOpen(value)
        }
      >
        <DialogContent className="w-[calc(100%-2rem)] max-w-md rounded-xl">
          <DialogHeader>
            ...
            <DialogDescription>
              El paciente saldrÃ¡ de la cola, se cancelarÃ¡ la cita anterior y
              se crearÃ¡ un nuevo registro. Esta acciÃ³n conservarÃ¡ la
              trazabilidad.
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog
        open={deleteOpen}
        onOpenChange={(value) =>
          !deleteMutation.isPending && setDeleteOpen(value)
        }
      >
        <DialogContent className="w-[calc(100%-2rem)] max-w-md rounded-xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Esta acción eliminará permanentemente la atención y no podrá
              recuperarse. ¿Deseas continuar?
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
